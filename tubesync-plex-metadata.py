...
def setup_ffmpeg():
    ...
    try:
        ...
        if not FFMPEG_BIN.exists(): sys.exit(1)
...
def extract_subtitles(video_path):
    ...
    try:
        ...
        for s in streams:
            ...
            subprocess.run(["ffmpeg","-y","-i",video_path,f"-map","0:s:{idx}",srt], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
...
def apply_nfo(ep, file_path):
    ...
    if not always_apply_nfo and cached_hash == nfo_hash:
        if detail: print(f"[DEBUG] NFO hash unchanged, skipping: {nfo_path}")
        return False
    try:
        ...
        try:
            nfo_path.unlink()
            if detail: print(f"[DEBUG] Deleted NFO: {nfo_path}")
        except Exception as e:
...
def scan_and_update_cache():
    ...
    for lib_id in config["plex_library_ids"]:
        ...
        for location in getattr(section, "locations", []):
        ...
        if nfo_path.exists() and nfo_path.stat().st_size > 0:
            ...
            if always_apply_nfo or cached_hash != nfo_hash:
                ...
                try:
                    ...
                    plex_item = find_plex_item(f)
                    if plex_item: cache[f]["plex_id"] = plex_item.key
                if plex_item: apply_nfo(plex_item, f)
                cache[f]["nfo_hash"] = nfo_hash
                cache_modified = True
...
def process_file(file_path, ignore_processed=False):
    ...
    if str_path in cache:
        ...
        if plex_id:
            try: plex_item = plex.fetchItem(plex_id)
    ...
    if nfo_path.exists() and nfo_path.stat().st_size > 0 and plex_item:
        if not silent: print(f"[INFO] Applying NFO: {abs_path}")
...
class VideoEventHandler(FileSystemEventHandler):
    ...
    def process_nfo_queue(self):
        ...
        for nfo_path in queue:
            ...
            if video_path.exists():
                futures.append(self.executor.submit(self.apply_nfo_with_subtitles, nfo_path, video_path))
    ...
    def process_video_queue(self):
        ...
        futures = [self.executor.submit(process_file, vp, True) for vp in queue]
        for fut in as_completed(futures): fut.result()
        save_cache()

    def apply_nfo_with_subtitles(self, nfo_path, video_path):
        plex_item = find_plex_item(str(video_path))
        if plex_item:
            apply_nfo(plex_item, str(video_path))
            update_cache(str(video_path), plex_item.key, compute_nfo_hash(nfo_path))
            try: Path(nfo_path).unlink()
            except: pass
...
def main():
    ...
    if threads > 1:
        ...
        for f in all_videos: process_file(f)
