...
# Argument parsing
# ==============================
parser = argparse.ArgumentParser(description="TubeSync Plex Metadata & Subtitles")
...
subtitles_enabled = config.get("subtitles", True)
...
def apply_nfo(ep, file_path):
    ...
    if not always_apply_nfo and cached_hash == nfo_hash:
        if detail:
...
def extract_subtitles(video_path):
    ...
    try:
        ...
        for s in streams:
            ...
            ext = codec if codec else "srt"
            out_file = f"{base}.{lang}.{ext}"
            if os.path.exists(out_file):
                continue
            subprocess.run([str(FFMPEG_BIN), "-y", "-i", video_path, "-map", f"0:s:{idx}", out_file],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
            if os.path.exists(out_file):
                subtitle_files.append((out_file, lang))
    except Exception as e:
        logging.error(f"[ERROR] Failed to extract subtitles from {video_path}: {e}")
    return subtitle_files

def upload_subtitles(ep, subtitle_files):
    for sub_file, lang in subtitle_files:
        try:
            with api_semaphore:
                ep.uploadSubtitles(sub_file, language=lang)
                time.sleep(request_delay)
            if detail:
                logging.info(f"[SUBTITLE] Uploaded: {sub_file}")
        except Exception as e:
            logging.error(f"[ERROR] Subtitle upload failed: {sub_file} - {e}")
...
def process_file(file_path, ignore_processed=False):
    ...
    if not plex_item:
        ...
        if plex_item:
            ...
            update_cache(str_path, plex_item.key, nfo_hash)
    ...
    if nfo_path.exists() and nfo_path.stat().st_size > 0 and plex_item:
        if not config.get("silent", False):
            logging.info(f"[INFO] Applying NFO: {abs_path}")
        if apply_nfo(plex_item, str_path):
            success = True

    # Extract & upload subtitles
    if subtitles_enabled and plex_item:
        subs = extract_subtitles(str_path)
        if subs:
            upload_subtitles(plex_item, subs)
...
def scan_and_update_cache():
    ...
    for f in all_files:
        if f not in cache:
            ...
            if plex_item:
                update_cache(f, plex_item.key)

        nfo_path = Path(f).with_suffix(".nfo")
        if nfo_path.exists() and nfo_path.stat().st_size > 0:
            nfo_hash = compute_nfo_hash(nfo_path)
            cached_hash = cache.get(f, {}).get("nfo_hash")
            if always_apply_nfo or cached_hash != nfo_hash:
                plex_item = find_plex_item(f)
                if plex_item:
                    apply_nfo(plex_item, f)
                    update_cache(f, plex_item.key, nfo_hash)
...
class VideoEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        if ext in VIDEO_EXTS:
            self._debounce(self.video_queue, self._process_videos, event.src_path)
        elif ext == ".nfo":
            self._debounce(self.nfo_queue, self._process_nfos, event.src_path)

    def _debounce(self, queue, callback, path):
        with self.lock:
            queue.add(path)
            if getattr(self, f"{callback.__name__}_timer", None):
                getattr(self, f"{callback.__name__}_timer").cancel()
            timer = threading.Timer(watch_debounce_delay, lambda: callback())
            setattr(self, f"{callback.__name__}_timer", timer)
            timer.start()
...
def main():
    ...
    if config.get("watch_folders", False):
        observer = Observer()
        for p in [Path(loc) for lib_id in config["plex_library_ids"]
                  for loc in getattr(plex.library.sectionByID(lib_id), "locations", [])]:
            observer.schedule(VideoEventHandler(), str(p), recursive=True)
        observer.start()
        try:
            while True:
                time.sleep(1)
