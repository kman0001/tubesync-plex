    ...
    try:
        ...
        for s in streams:
            ...
            if os.path.exists(srt):
                continue
            subprocess.run(
                ["ffmpeg","-y","-i",video_path,"-map",f"0:s:{idx}",srt],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
            )
            if os.path.exists(srt):
                srt_files.append((srt, lang))
    except Exception as e:
        print(f"[ERROR] Subtitle extraction failed: {video_path} - {e}")
    return srt_files

def upload_subtitles(ep, srt_files, semaphore, request_delay, detail=False):
