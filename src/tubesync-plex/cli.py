...
from tubesync_plex.metadata import scan_and_update_cache, process_file, save_cache
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=config.get("threads",4)) as ex:
        futures = {ex.submit(process_file, f, plex, config): f for f in config["cache"].keys()}
